# research_agent_magic.py — OpenAI + citations + LaTeX support
# -------------------------------------------------------------
...
def _default_setup_if_missing(ns):
    ...
    # --- 2️⃣ Load course materials and build the RAG index ---
    ...
    print(
        ...
        f"tools so far: {len(ns['initial_tools'])}, llm: OpenAI"
    )
...
def _run_tool_by_name(ns, tool_name: str, prompt: str):
    ...
    tools = ns.get("initial_tools", []) or []
    tool = next((t for t in tools if getattr(t, "metadata", None)
                 and getattr(t.metadata, "name", "") == tool_name), None)
    if tool is None:
        ...
        try:
            ...
            async def run_with_retry(prompt_text):
                enforced_prompt = (
                    ...
                    "Always respond in English and render any math expressions using LaTeX. Use '$...$' for inline math and '$$...$$' for block equations.\n\n"
                    + prompt_text
                )
                for _ in range(3):
                    try:
            ...
            if course_tool:
                ...
                context_prompt = (
                    ...
                    "Write all mathematical equations in proper LaTeX syntax, using '$...$' for inline math and '$$...$$' for block equations. "
                    "If the answer contains any formula or expression, always render it using LaTeX.\n\n"
